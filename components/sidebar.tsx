...
export function Sidebar() {
  return (
    <div className="flex h-full flex-col gap-y-5 border-r border-gray-200 bg-[#33B04F] px-6 dark:border-slate-600 dark:bg-slate-800">
      <nav className="flex flex-1 flex-col pt-40 2xl:pt-48">
        <ul role="list" className="flex flex-1 flex-col gap-y-5">
          <span className="text-center text-2xl font-medium text-white">
            MACROTIPOS
          </span>
          <li>
            <ul role="list" className="-mx-2 space-y-4 2xl:space-y-6">
              {navigation.map((item) => (
                <li key={item.name}>
                  <Link
                    href={item.href}
